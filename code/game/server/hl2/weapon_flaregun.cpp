...
{
#ifdef EXTENDED_FIRE
	CFlare *pFlare = CFlare::Create(vOrigin, Angles, pOwner, flDuration, goalHeatLevel);
#else
...
//-----------------------------------------------------------------------------
#ifdef EXTENDED_FIRE
CFlare *CFlare::Create(Vector vecOrigin, QAngle vecAngles, CBaseEntity *pOwner, float lifetime, float goalHeatLevel)
#else
...
#ifdef EXTENDED_FIRE
	CFlare *pFlare = CFlare::Create(pOwner->Weapon_ShootPosition(), pOwner->EyeAngles(), pOwner, FLARE_DURATION, 32); // todo: turn into a skill convar?
#else
...
#ifdef EXTENDED_FIRE
	CFlare *pFlare = CFlare::Create(pOwner->Weapon_ShootPosition(), pOwner->EyeAngles(), pOwner, FLARE_DURATION, 32);
#else
	CFlare *pFlare = CFlare::Create( pOwner->Weapon_ShootPosition(), pOwner->EyeAngles(), pOwner, FLARE_DURATION );
#endif
